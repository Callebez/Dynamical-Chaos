                                ...
                                double step, double param)
{
    ...
    std::vector<double> timeEvolution;
    ...
    completeRungeKutta45(function,initialCond, rk45, param,step,coord.size(),tol, time, timeEvolution);
    ...
    for(uint i = 0; i < rk45.size(); i++)
    {
        ...
        //of the eigenvectors  
        J = jacobian(coord, param,timeEvolution[i]);
        w = matMult(J,w);
                                            ...
                                            std::vector<std::vector<double>>& lyapunovSpectrumRange)
{
   ...
   //inicialization for the integration
    ...
    {
        lyapunovExponents.emplace_back( lyapunovSpectrum(function,jacobian,initialCond,tol,time,integrationStep,*paramValue));
    }
    for(uint i = 0; i < param.size();i++)
    {
        lyapunovSpectrumRange[i][0] = param[i];

        for(uint j = 1; j < initialCond.size() + 1; j++)
        {
